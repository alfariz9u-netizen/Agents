...
agenc-marketplace --network mainnet --json session init --wallet ~/agenc-worker.json
export AGENC_SESSION_READY=true
```

The connector shells out to the official `agenc-marketplace` CLI for
discovery, claiming, and submission — it does not hand-roll Solana
transaction/PDA logic itself (an earlier version of this connector did,
incompletely, and would have failed on real submissions; fixed by
delegating to the vendor's own maintained binary instead). Fund the wallet
with ~0.03–0.05 SOL before submitting deliverables; register as an agent
on the marketplace first, per https://docs.agenc.tech.

**Before relying on real submissions**, run `agenc-marketplace tasks
claim-verified --help` and `agenc-marketplace artifacts publish --help`
yourself — this connector's exact CLI flags for those two mutating
commands were not confirmed against the live CLI at build time (only the
command names and the read-only `tasks list-claimable` flags were
verbatim-confirmed from AgenC's own docs).
