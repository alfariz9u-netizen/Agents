...
| AgenC | `agenc-marketplace` CLI installed + a funded Solana keypair via `session init` (`AGENC_SESSION_READY=true`) | Real SOL (tx fees + rent) |
...
- **Fixed: AgenC connector never actually worked for submission.** The original `src/connectors/agenc.js` called `@tetsuo-ai/marketplace-sdk`'s `submitTaskResult()` directly, with a code comment admitting task/agent PDA (Program Derived Address) resolution was never implemented — every real submission would have failed on-chain. Researched AgenC's own documentation (github.com/tetsuo-ai/AgenC) and found they ship an official CLI/MCP binary, `agenc-marketplace`, that owns PDA derivation, transaction building, and signing itself, with confirmed real command syntax (`tasks list-claimable`, `tasks claim-verified`, `artifacts publish`, `session init`). Rewrote the connector to shell out to this official binary instead of hand-rolling Anchor/PDA logic — the correct fix, not a workaround, since reimplementing a live-mainnet program's PDA scheme from partial docs is exactly the kind of thing that should be delegated to the vendor's maintained tool. Also removed the now-unnecessary `@tetsuo-ai/marketplace-sdk`/`@solana/kit` dependencies entirely. Proven in `test/agenc_cli.test.js` against a fake CLI binary (same testing pattern as the AgentBazaar Python bridge) — discovery, claim, submit, and both failure modes (bad invocation, missing binary) all produce real, honest results. Remaining honesty gap: the exact CLI flags for the two mutating commands (`claim-verified`, `artifacts publish`) beyond their command names were not confirmed against the live binary's `--help` output at build time — flagged explicitly in the connector's own comments and in `docs/setup.md`.
